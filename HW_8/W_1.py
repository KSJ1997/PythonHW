...
def output_data(file_name: str, rows):
    output_entries = list()
    if len(rows) > 0:
...
def find_data(file_name: str, search_query: str):
    found_items = list()
...
def delete_data(file_name: str, at_row: int):
    entries = get_entries(file_name)
    if 0 <= at_row < len(entries):
        entries.pop(at_row)
...
def switchboard():
    # display_menu()
    mode = int(input('Введите номер опцию: '))
    while True:
        match mode:
            case 0:
            ...
            case 4:
                ...
                delete_id = int(
                    input('Введите ID номер записи которую нужно удалить: '))
